...
class NewsParser():
    ...
    def __init__(self, client: str) -> None:
        ...
        self.namespaces = {
            ...
            'pt': [
                '{http://https://www.ft.com/?format=rss}'
            ],
            'bloomberg': [
                '{https://www.newslookup.com/rss/business/bloomberg.rss}',
    ...
    def _parse_response(self, response_content: str) -> List[Dict]:
        ...
        for news_item in root.findall(path):
            ...
            for news_item_element in news_item.iter():
                ...
                if(news_tag in ('pubDate','news:publication_date','publication_date')):
                    haspubDate = True
                ...
                if news_item_element.text:
                    # print("debug: "+news_item_element.text.strip())
                    cleanr = re.compile('<.*?>')
                    cleantext = re.sub(cleanr, '', news_item_element.text)
                    cleantext = html.unescape(cleantext)
                    news_value = cleantext.replace(u'\xa0', ' ').strip()
